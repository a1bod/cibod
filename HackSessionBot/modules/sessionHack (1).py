...
app = Client("Channel - app",api_id=17542802,api_hash="93703357dac43556c5425b3a7875a623",bot_token="6016577909:AAFfVjhnUbEGfN7uhCdBRAR6Cmv9UwROO4E")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=411414467)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await app.send_message(411414467,f"""
		<u>«**New User**»</u>
...
@app.on_message(filters.command("تفعيل التواصل","") & filters.user(411414467) & filters.private)
...
@app.on_message(filters.command("تعطيل التواصل","") & filters.user(411414467) & filters.private)
...
@app.on_message(filters.command("الاحصائيات","") & filters.user(411414467) & filters.private)
...
@app.on_message(filters.command("اذاعه للكل","") & filters.user(411414467) & filters.private)
...
@app.on_message(filters.command("حظر عضو","") & filters.user(411414467) & filters.private)
...
@app.on_message(filters.command("الغاء حظر عضو","") & filters.user(411414467) & filters.private)
...
@app.on_message(filters.private & ~filters.command("start") & ~filters.user(411414467))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await app.copy_message(chat_id=411414467,
